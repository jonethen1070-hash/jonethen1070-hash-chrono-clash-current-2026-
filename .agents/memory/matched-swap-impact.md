...
When board resolution is synchronous, a swapped piece may already be absent from the authoritative board by the next render pass. The renderer must preserve the actual committed swap IDs and destination socket coordinates, then animate any removed IDs as dying overlays before beginning the match charge/break and refill.

**Why:** Inferring swaps from final adjacency misses pieces cleared in the same resolve and makes valid matches appear to pop out before completing the player’s gesture-linked exchange.
